---
title: "CloudWatch Logs のコストを月 100 万円削減した (EKS × fluentbit)"
emoji: "📉"
...
[TROCCO](https://trocco.io) という顧客のデータを 1 日 25 万回以上 ETL 転送するプラットフォームで、CloudWatch Logs のコストを月 100 万円削減したため、事例の共有です。(ドル円 160 円換算)


以下は CloudWatch Logs のみを選択肢した Cost Explorer の図です。
大きくコスト削減できていることが分かります。

![](https://static.zenn.studio/user-upload/2365e94817ab-20260814.png)

今回、1 個の改善ではなく、いくつかの施策を組み合わせることで、100 万円/月のコスト削減を達成しました。[^1]
...
本記事では、汎用性が高い fluentbit のアプローチについて事例をご紹介します。

[^1]: fluentbit だけの解説であれば月 50 万分の削減のため、ややタイトル詐欺ですが、グラフを見ていただけると分かる通り、実際に一連の流れで月 100 万近く削減し、月 100 万削減したうちの 1 つを紹介します、という位置づけなので、タイトルは月 100 万としています。


## 初期のログ構造

ETL 処理を担うデータ転送ジョブは EKS の Kubernetes Job で動いており、標準出力を CloudWatch Logs に送っています。

1 つのデータ転送で 1 つの Kubernetes Job を動かすため、データ転送ごとに pod は分離しています。

ログに pod の情報を付与するために、CloudWatch Logs への送信前に fluentbit の [Kubernetes フィルタ](https://docs.fluentbit.io/manual/data-pipeline/filters/kubernetes) で Kubernetes の情報を付与しています。
...
具体的にはアプリケーション内で `logger.info` を数回実行しても、fluentbit 側でバッファリングして複数行をまとめて 1 イベントとして CloudWatch Logs に送る仕組みです。
...
3 秒ごとの flush のため、実際のログの時刻と、CloudWatch Logs の timestamp が 3 秒ほどズレてしまうデメリットがありますが、コストメリットの方が大きいためデメリットを受容することにしています[^3]。
...
fluentbit には emitter という内部の組み込みプラグインがあり、設定したフィルタの意図せぬ重複実行リスクがあります。

この挙動が複雑で、multiline 化したログを buffer flush するタイミングで、emitter がログを fluentbit の ログパイプラインの先頭に再出力します。

fluentbit の[公式ドキュメント](https://docs.fluentbit.io/manual/data-pipeline/filters/multiline-stacktrace)にも強い注意書きがあります。
(重複実行だけでなく無限ループリスクも！)
...
しかし、我々は multiline filter をパイプラインの最初には定義せず、`truncate_massive_log` (1 行の長さ切り詰め) -> multiline フィルターの順番で定義しました。
...
multiline 化実装後、fluentbit のメモリ使用量が大きく増えました。(実装中も少し嫌な予感を持っていました)
...
実際にメトリクスを観測していると、メモリ使用量が pod の limit に近づいた場合、OOM ではなく、メモリ使用量が大きく減る(キャッシュが解放される)挙動をし、これが繰り返されるだけ、かつ、いくつか pod を調査したところ  `slab_reclaimable` が支配的であることを観測したため、特にリソースチューニングせずとも問題なく動いています。[^4]
...
Lua スクリプトで、multiline 化した後のログサイズが 200KB よりも大きなサイズになった場合、アラート用のメタデータをログに付与し、CloudWatch Logs メトリクスフィルター + CloudWatch Alarm で通知する仕組みです。
...
加えて fluentbit 以外に同タイミングで行ったその他の改善の積み重ねで、最終的に **100 万円/月** のコスト削減を達成しました。(ドル円 160 円換算)
