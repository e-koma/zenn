...
Fluent Bit の Kubernetes フィルタが付与するメタデータは地味にデータ量が多く、CloudWatch Logs のコストに直結します。

不要なメタデータを削り、複数行を 1 イベントにまとめて送ることでメタデータの付与回数を削減することで、月 50 万円のコスト削減を実現しました。
...
顧客データの ETL 処理を担うデータ転送ジョブは EKS の Kubernetes Job として動き、標準出力を CloudWatch Logs に送っています。1 つのデータ転送につき 1 つの Kubernetes Job を起動するため、pod はデータ転送ごとに分離しています。
...
上記はログサイズが大きすぎる warning です。CloudWatch Logs は 1MB を超えるログを受け付けません[^2]。

[^2]: アプリケーション内で 1MB ものログを出力するような箇所はありません。調査すると、顧客の特殊な環境に依存した大規模なログが原因で 1 行 1MB 以上のログが出力されていました。

顧客に提供する実行ログは CloudWatch Logs 以外の別経路で保存しており、Fluent Bit を通るログはシステム運用管理者向けの調査ログ用途のため、不要なものは積極的に削ることができます。
...
- ログを 1 行 1KB に切り詰める
- multiline 化する (複数行を \n で結合)
- multiline 化したログ全体を 1KB に切り詰めることで、ログ欠損になる。

という問題が起きてしまいました。

### 副作用その2への対処

Fluent Bit の[公式ドキュメント](https://docs.fluentbit.io/manual/data-pipeline/filters/multiline-stacktrace)には、「multiline をパイプラインの先頭に置けば、フィルタの重複実行を避けられる」とあります。

しかし我々はこの方針は採りませんでした。パイプラインの順序はそのままに(1 行切り詰め -> multiline 化)、1 行切り詰めのフィルタが 2 回実行されても壊れないよう、処理を冪等にしました。

multiline を先頭にすると、まだ切り詰めていないサイズ不明の巨大な 1 行を、複数行分バッファしなければならず、バッファサイズを大きめに設計する必要があります。

一方、切り詰めを先に行えば、バッファに入るのはすでに短くなった行だけです。フィルタの再実行リスクは残りますが、処理を冪等にしておけば問題になりません。バッファに溜まるものが意図したサイズに揃っている方が、挙動をコントロールしやすく、バッファも小さく保てて安定すると判断しました。

1 行切り詰めの `truncate_massive_log` は以下のように修正しました[^5]。
...
一見、OOM リスクがあるようにも思えましたが、増えたメモリの中身 (`/sys/fs/cgroup/memory.stat`) をよく見ると、メモリキャッシュが溜まっているだけで、ほとんど問題ないことが分かりました。
...
1 行切り詰めや multiline は、効いているつもりでも壊れたことを検知するため
以下の条件で CloudWatch Alarm が通知されるようにしました。

- Fluent Bit が巨大ログを切り詰め・破棄する warning を出したとき (warning が再発した場合)
- バッファ上限を 200KB にしているのに、送信サイズが大きく上回ったとき

後者は、multiline 後のログサイズを Lua で見て、閾値を超えたレコードにアラート用のメタデータを付け、CloudWatch Logs のメトリクスフィルタ経由で Alarm する仕組みです。
...
加えて Fluent Bit 以外に同タイミングで行ったその他の改善の積み重ねで、
最終的に **100 万円/月** のコスト削減を達成しました。(ドル円 160 円換算)
